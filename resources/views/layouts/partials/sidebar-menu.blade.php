...
<a href="{{ route('dashboard') }}" class="nav-link {{ str_starts_with($currentRoute, 'dashboard') ? 'active' : '' }}">
...
@elseif($role === 'admin')
    ...
    <a href="#" class="nav-link {{ str_starts_with($currentRoute, 'admin.roles') ? 'active' : '' }}">
    ...
    <a href="#" class="nav-link {{ str_starts_with($currentRoute, 'admin.email') ? 'active' : '' }}">
        <i class="fas fa-envelope"></i> Email
    </a>
    <a href="#" class="nav-link {{ str_starts_with($currentRoute, 'admin.notifications') ? 'active' : '' }}">
    ...
    <a href="#" class="nav-link {{ str_starts_with($currentRoute, 'admin.monitoring') ? 'active' : '' }}">
...
@if (in_array($role, ['student', 'teacher', 'parent', 'secretary', 'admin']))
    ...
    <a href="#" class="nav-link {{ str_starts_with($currentRoute, 'messages') ? 'active' : '' }}">
        <i class="fas fa-comments"></i> Mensagens
        @if ($unreadMessagesCount > 0)
            <span class="badge bg-danger badge-notification">{{ $unreadMessagesCount }}</span>
        @endif
    </a>
    <a href="#" class="nav-link {{ str_starts_with($currentRoute, 'blog') ? 'active' : '' }}">
        <i class="fas fa-newspaper"></i> Blog/Notícias
    </a>
    <a href="#" class="nav-link {{ str_starts_with($currentRoute, 'events') ? 'active' : '' }}">
        <i class="fas fa-calendar-check"></i> Eventos
    </a>
    <a href="#" class="nav-link {{ str_starts_with($currentRoute, 'calendar') ? 'active' : '' }}">
...
<a href="#" class="nav-link {{ str_starts_with($currentRoute, 'profile') ? 'active' : '' }}">
    <i class="fas fa-user"></i> Meu Perfil
</a>
<a href="#" class="nav-link {{ str_starts_with($currentRoute, 'settings') ? 'active' : '' }}">
    <i class="fas fa-cog"></i> Configurações
</a>

<!-- Ajuda -->
<div class="nav-section-title mt-4">Suporte</div>
<a href="#" class="nav-link {{ str_starts_with($currentRoute, 'help') ? 'active' : '' }}">
    <i class="fas fa-question-circle"></i> Ajuda
</a>
<a href="#" class="nav-link {{ str_starts_with($currentRoute, 'contact') ? 'active' : '' }}">
